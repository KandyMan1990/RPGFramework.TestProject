namespace Test.Menu.Localisation
{
	// Auto generated keys for sheet: BeginMenu
	public static partial class LocalisationKeys
	{
		public static class BeginMenu
		{
			///<summary>BeginMenu</summary>
			public const string SHEET_NAME = @"BeginMenu";
			///<summary>New_Game</summary>
			public const string NEW_GAME = @"BeginMenu/New_Game";
			///<summary>Load_Game</summary>
			public const string LOAD_GAME = @"BeginMenu/Load_Game";
			///<summary>Quit_Game</summary>
			public const string QUIT_GAME = @"BeginMenu/Quit_Game";
		}
	}
}
